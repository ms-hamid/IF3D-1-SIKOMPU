...
def predict():
    ...
    try:
        ...
        df = df.sort_values(by='skor_prediksi', ascending=False).reset_index(drop=True)
        df['rank'] = df.index + 1

        # Pilih kolom yang ingin dikembalikan (pakai nama asli)
        hasil = df[['id', 'nama_asli', 'kode_matkul', 'skor_prediksi', 'rank']].rename(columns={'nama_asli':'nama'}).to_dict(orient='records')
